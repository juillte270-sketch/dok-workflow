...
- 단가와 금액은 원(₩) 단위 정수
- unit_price는 반드시 1개당 단가. total = unit_price × qty. 수량이 2개 이상이면 반드시 나눠서 1개 단가를 unit_price에 입력
- 수량이 명확히 표시되지 않으면 qty=1, unit_price=total로 설정
- 합계/부가세/총액 행은 제외
- 금액이 0이거나 빈 행은 제외

손글씨 가격 해석 (매우 중요):
- "13,-" 또는 "13,―" 또는 "13.-" 표기는 13,000원을 의미 ("-"는 000의 약어)
- "10,5-" 또는 "10.5-" 에서 숫자 아래 밑줄/작대기가 있으면 → 10,500원 (5 아래 줄 = "00")
- "2,5-" 에서 5 아래 밑줄 → 2,500원
- 숫자 뒤의 "-" 는 항상 뒤에 0이 붙는다는 표시 (밑줄 없으면 "000", 밑줄 있으면 "00")

공급처별 품목명 힌트:
- 영운농산(손글씨): "벌크채"=깻잎(1kg), "랍바라"=깻잎(4K,큰잎), "적근대" 아래 "벌크채"가 보이면 별개 품목
- 이모유통(손글씨): "완숙"=완숙토마토
- 태현상회(손글씨): "깐 42"=깐양배추42, "깐 45"=깐양배추45. 통 단위 기재 시 1통 단가를 unit_price로
- 명화농산(손글씨): "배추"와 "양배추"는 다른 품목. 배추 1통 가격은 보통 4,000원대, 양배추42는 2,500원대

거래명세서 양식의 컬럼: 월/일, 품목, 단위, 수량, 단가, 공급가액 — 단가 컬럼의 값을 unit_price로 사용
손글씨가 흐리거나 약어로 쓰여 있어도 최대한 추정하여 읽어주세요. 모든 품목을 빠짐없이 추출하세요."""
